...
class Help(commands.Cog):
    ...
    @commands.command()
    async def help(self, ctx: commands.Context, help_input: str = None):
        """Help command"""

        if help_input is None:
            embed = discord.Embed(title='Categories and commands', color=discord.Color.blue(),
                                  description=f'Use `{ctx.prefix}help <category|command>` to gain more information '
                                              f'about a category or command')
            ...
            cog = self.bot.get_cog(help_input.title())
            if cog is not None:
                embed = discord.Embed(title=cog.qualified_name, color=discord.Color.gold(),
                                      description=f'Use `{ctx.prefix}help <command>` to gain more information about a '
                                                  f'specific command')
                cog_commands = cog.get_commands()
                cog_commands_text = ''
                for command in cog_commands:
                ...
                command = self.bot.get_command(help_input)
